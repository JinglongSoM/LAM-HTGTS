...
my $join_min_ol = 30;
my $skipclean;
my $forward_adapter = "AGATCGGAAGAGCACACGTC";
my $reverse_adapter = "AGATCGGAAGAGCGTCGTGT";
my $adapter_fa = "$FindBin::Bin/../ref/IlluminaAdapters-PE.fa";
my $use_seqprep = 0;   # whether to use SeqPrep or cutadapt for triming. 0 is False
...
sub process_experiment ($) {
	...
	unless (defined $no_trim) {
        ...
        if ($use_seqprep) {
            ...
            System("echo \"Running cutadapt on $expt\" >> $logfile",1);
            my $trim_cmd = join(" ","cutadapt ",$meta{$expt}->{R1}," ",$meta{$expt}->{R2}, "-o",$meta{$expt}->{R1trim},"-p",$meta{$expt}->{R2trim}, "--minimum-length=$minlen -a $forward_adapter -A $reverse_adapter >> $logfile 2>&1");
            System("echo \"$trim_cmd\" >> $logfile",1);
            System($trim_cmd,1) or croak "Error: failed running cutadapt on $expt";
        }
